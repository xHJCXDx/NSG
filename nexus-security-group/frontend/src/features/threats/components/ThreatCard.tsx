    ...
    <article className="glass-card p-5 space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold text-white">{threat.category ?? 'Threat detection'}</h2>
          <p className="text-xs text-gray-500">Detected: {formatDate(threat.detectedAt)}</p>
        </div>
      ...
      {threat.evidence && threat.evidence.length > 0 && (
        <div>
          <p className="text-sm font-semibold text-gray-300">Evidence</p>
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm text-gray-400">
        ...
        {threat.relatedMention ? (
          <span>
            Related mention: {threat.relatedMention.text ?? threat.relatedMention.id}
            {threat.relatedMention.platform ? ` (${threat.relatedMention.platform})` : ''}
          </span>
        ) : threat.mentionId ? (
          <span>Related mention ID: {threat.mentionId}</span>
        ) : (
          <span>No related mention provided</span>
        )}
      </div>
    </article>
  );
