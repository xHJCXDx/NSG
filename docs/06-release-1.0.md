...
- [x] `auth`: confirmar que rutas privadas dependan de usuario autenticado o admin según corresponda, y documentar cualquier ruta pública intencional. Contrato Release 1.0 confirmado: todas las rutas privadas bajo `metrics`, `n8n`, `threats`, `alerts`, `keywords`, `logs`, `activity` y `dashboard` dependían de `get_current_user`; `users` era admin-only mediante helper legacy; las únicas rutas públicas intencionales eran `POST /api/auth/login` para intercambio de credenciales y `GET /api/health` como health probe operativo. Release 2.0 migró estas rutas a permisos granulares.
...
- [x] Revisar rutas protegidas manualmente o por tests: `test_route_auth_contract.py` cubre exhaustivamente todas las rutas `/api/*` — en Release 1.0 verificaba que cada endpoint privado tuviera dependency de autenticación/autorización explícita, detectaba rutas no documentadas y confirmaba que solo `POST /api/auth/login` y `GET /api/health` eran públicas intencionalmente. Release 2.0 actualizó el contrato a permisos granulares.
